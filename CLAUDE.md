...
A static-HTML Korean coding-education site (초등·중학생 대상) that teaches block coding with **Entry** (entry.org.kr — a Korean Scratch-like platform). Content is a 12-part series that builds physics-based game movement step by step (이동·충돌 → 속도·가속도 → 관성·마찰 → 포물선·발사 → 튕김·탄성 → 힘·질량 → 운동량·충돌 → 에너지 → 경사면·마찰 → 스프링·진동 → 원운동 → 인공위성), plus a TOC and an "엔트리란?" intro page.
...
├── entry-game-10-spring.html   # 10편 (sage accent, --c10) — spring oscillation (pull-release) demo
├── entry-game-11-circular.html # 11편 (terracotta accent, --c11) — circular-motion (orbit + cut-string) demo
├── entry-game-12-satellite.html# 12편 (midnight-blue accent, --c12) — satellite/orbit (Newton's cannon, real 1/r² gravity) demo
...
- **Tip/callout boxes**: `.tip`, `.tip.warn`, `.tip.bonus`, plus `.tip.real` (「현실 vs 게임」 구분 콜아웃), `.goals` (편 상단 학습 목표 — 🔬물리 개념 / 🧩엔트리 구현 2칼럼), and `.fix` (「안 될 때 확인!」 오류 해결 박스). The 물리 개념 ↔ 엔트리 구현 distinction is a canonical content principle — see design-guide D-4 (rhythm), authoring-editing-guide A-3 (accuracy table), new-page-guide §2/§4 (skeleton/checklist).
...
1. Copy `docs/entry-game-2-velocity.html` as the base (most up-to-date CTA/TOC/meta structure). For the 개념↔구현 분리 structure itself, `docs/entry-game-3-inertia.html` is the reference (design-guide D-4).
2. Fill in the `<!-- FILL -->` placeholders: title, badge, h1, sections, footer episode label.
3. Add a `.goals` box right after the intro, splitting 🔬 physics concepts from 🧩 Entry implementation, then structure the body as 🔬 concept-reading → 🧩 follow-along-building. Use `.tip.real` where a metaphor diverges from real physics, `.fix` where learners get stuck, and `.tip.bonus` ⭐도전 for high-difficulty bits.
4. Pick or assign an accent token (`--c1`…`--c5`/`--green`/`--accent`) and apply via `section.card.c-{color}` and the page-specific `.cta` style.
5. Add a `.cta` block (heading + description + colored `<a class="go">` button) at the end of the content as the bottom navigation. Middle episodes' `.go` button links to the next page; the last episode links to `index.html` with "🏠 목차로 돌아가기".
6. Keep the `<nav class="toc-sidebar">` element + TOC script block at the very end of `<body>` — it auto-collects every `section.card h2` into a sticky right sidebar on wide screens (≥1240 px). No changes needed to the script itself.
7. Add the page to `docs/index.html` `.toc-grid` and update the previous episode's CTA button to point to the new page.
8. No router, no build — links are plain relative paths (`href="entry-game-X-foo.html"`).
