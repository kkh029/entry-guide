...
### 보너스 「3D는 어떻게 보일까?」
출처: [about-3d-depth-cues.html](about-3d-depth-cues.html)

- [ ] **실기기/visible 탭에서 단서 토글 데모 입력 반응 최종 확인**
  거리 슬라이더·체크박스 5개·‘전부 켜기’ 토글이 실기기에서 매끄러운지(헤드리스 스크린샷은 정적 렌더만 검증).
- [ ] **(선택) ★도전을 별도 페이지로 — 아이소메트릭 쿼터뷰·회전 정육면체(원근 투영) 데모**
  현재는 `.tip.bonus` 미리보기만. 반응이 좋으면 좌표변환/삼각함수 데모로 확장.
- [ ] **(선택) 데모에 ‘시차(parallax) 배경 레이어’ 6번째 단서 추가 검토**
  깊이 단서를 하나 더 보여주나 정보량↑로 단순함과 트레이드오프 — 보류.
